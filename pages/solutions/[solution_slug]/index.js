...
const SolutionPage = (props) => {
    ...
    const{ solution_slug } = router.query;
    const [solution, setSolution] = useState()
    const [error, setError] = useState();

    useEffect(() => {
        if (solution_slug !== undefined){
            const viewSolutionURL = FermatUrls.viewSolution;
            const token = getUserToken();
            axios.get(viewSolutionURL, {
                params: {
    ...
    if (!solution) return <Loading height="ht-100p"/>

    return (
        <>
            <Head
                title={`${solution && solution.title} Solutions by Fermat | Fermat | StdAux`}
                description={solution.long_description}
            />
                ...
                <div className="mg-0 pos-relative pd-0 ht-lg-600 ht-md-600 ht-xl-600 wd-100p">
                    <div className="pos-absolute t-100 r-100 z-index-10 bg-white pd-10 rounded d-none d-xl-block d-lg-block d-md-block">
                        <div className="ht-100p wd-400 d-flex flex-column justify-content-center pd-20">
                            <h3 className="tx-24 mg-b-15 mg-md-b-20 tx-bold">{solution && solution.title}</h3>
                            <Markdown>{solution && solution.short_description}</Markdown>
                            {solution &&
                                <>
                                {solution.whitepaper &&
                                    <a href={`${process.env.STDAUX_ROOT_URL}${solution.whitepaper.pdf}`} className="btn btn-primary btn-block wd-200">Get the White Paper</a>
                                }
                                </>
                            }
                        </div>
                    ...
                    <div className="mg-0 pd-0 ht-lg-600 ht-md-600 ht-xl-600 ht-xl-600 wd-100p">
                        <img src={`${process.env.STDAUX_ROOT_URL}${solution.image}`} className="img-fit-cover" alt=""/>
                    </div>
                </div>
                <div className="container pd-t-40">
                    <h1 className="tx-color-01 mg-b-5 center">Features</h1>
                    <p className="tx-color-03 tx-16 mg-b-40">{solution.long_description}</p>
                    {solution.features && solution.features.map((feature, index) => (
